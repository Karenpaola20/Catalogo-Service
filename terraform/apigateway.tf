...
resource "aws_api_gateway_integration" "get_payment_integration" {
  ...
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.payment_get.invoke_arn
}

resource "aws_lambda_permission" "allow_api_gateway_get_payment" {
  statement_id  = "AllowExecutionFromAPIGatewayGetPayment"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.payment_get.function_name
  principal     = "apigateway.amazonaws.com"
